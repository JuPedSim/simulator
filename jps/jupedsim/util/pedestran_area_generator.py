...
accuracy =0.01
# generates a
def calc_grid(polygon, distance):
    lines: list = []
    min_x, min_y, max_x, max_y = get_min_max(polygon)
    x_len = max_x - min_x
    y_len = max_y - min_y
...
def find_intersection(line_0, line_1):
...
def is_inside(point, polygon, min_x, min_y, max_x, max_y):
    intersections = count_intersections(point, polygon, min_x, min_y, -1, -1)
    if (intersections % 2) == 0:
        intersections = count_intersections(point, polygon, max_x, min_y, 1, -1)
    if (intersections % 2) == 0:
        intersections = count_intersections(point, polygon, min_x, max_y, -1, 1)
    return (intersections % 2) != 0


# gets the biggest/smallest x/y values of a list of points
def get_min_max(geometry):
...
def calc_boxes(points: list, distance: float):
    boxes: list = []
    n_found = 1
    for i, point in enumerate(points):
        ...
        for j, p2 in enumerate(points):
            if i == j: continue
            isright, isup = is_in_distance(point, p2, distance)
            if isup or isright:
                n_found += 1
                if isright:
                    box[3] = p2
                if isup:
                    ...
                    for k, p3 in enumerate(points):
                        if k == j: continue
                        isright, isup = is_in_distance(p2, p3, distance)
                        if isright:
                            n_found += 1
                            box[2] = p3
                            break
    ...
    for i, p0 in enumerate(points):
        ...
        while j < len(points):
            if i == j:
                j += 1
                continue
            if is_same_point(p0, points[j]):
                points.pop(j)
                j -= 1
            j += 1

def generate_spawn_points(polygon, distance):
    lines = calc_grid(polygon, distance)
    maybe_points, points = calc_intersections(lines, polygon)
    minx, miny, maxx, maxy = get_min_max(polygon)
    remove_duplicate_points(maybe_points)
    for i in range(len(maybe_points) - 1, -1, -1):
        if not is_inside(maybe_points[i], polygon, minx, miny, maxx, maxy):
            maybe_points.pop(i)
    points.extend(maybe_points)
    remove_duplicate_points(points)
    boxes = calc_boxes(points, distance)
    midpoints: list = []
    for box in boxes:
        midpoints.append(calc_midpoint(box, distance))
    return midpoints
