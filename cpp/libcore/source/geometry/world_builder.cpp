...
auto WorldBuilder::addSpecialArea(int p_level, int p_id, std::vector<int> p_segments) -> void
{
    // TODO: check if MA id already exists

    if(!p_segments.empty() && p_segments.size() % 4 != 0) {
        // TODO: throw exception. vector of segments must contain 4 points for each segment
    }
    // convert p_segments to vector of lines
    std::vector<Line> segment_lines(p_segments.size() / 4);

    // loop over segments (pair of points), 4 values each
    for(size_t i = 0; i < (p_segments.size() - 1); i += 4) {
        Point first_point{.x = p_segments[i], .y = p_segments[i + 1]};
        Point second_point{.x = p_segments[i + 2], .y = p_segments[i + 3]};
        segment_lines.push_back(Line{.start = first_point, .end = second_point});
    }

    // check if special area is a closed polygon. each start/end point of a line must be part of
    // another line Approach: loop over segments, search for point of this segment in the other
    // remaining segments if found: repeat loop with other point of found segment

    std::vector<Line> lines_to_check(segment_lines.begin() + 1, segment_lines.end());
    bool found_connection = false;
    Point current_point   = segment_lines[0].end;
    int remove_index      = -1;
    std::vector<Line> connected_lines(p_segments.size() / 4);
    ...
    do {
        ...
        for(auto it = lines_to_check.begin(); it != lines_to_check.end(); ++it) {
            ...
            if(it->end.x == current_point.x && it->end.y == current_point.y) {
        ...
        if(found_connection) {
            ...
            lines_to_check.erase(lines_to_check.begin(), lines_to_check.begin() + remove_index);
        }

    } while(found_connection);

    // check if all lines have been connected and last remaining point matches first point of first
    // segment
    if(lines_to_check.empty() && current_point.x == segment_lines[0].start.x &&
       current_point.y == segment_lines[0].start.y) {
